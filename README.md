...
For studies without re-sequenced files, the previous workflow still works by leaving empty the `Units` slot in the `config/config.yaml`, and specifying only `config/sample_table.csv`.
